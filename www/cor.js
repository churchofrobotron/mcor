...
var slogans = [
  'Praise the mutant!', 'Rejoice in error!', 'Embrace futility!',
  'Beware of sinister progs!', 'Take up the two 8 ways!',
  'Use the ninth position wisely!'
];
var colors = [
  'rgb(255, 0, 255)', 'rgb(0, 0, 255)', 'rgb(0, 255, 0)', 'rgb(255, 255, 0)'
];
...
var text_attrs = [
  {
    ...
    fade: 400,
    colors: [
      'rgb(255, 0, 255)', 'rgb(0, 0, 255)', 'rgb(0, 255, 0)',
      'rgb(255, 255, 255)'
    ]
  },
  {
...
function pageInit() {
  ...
  animatedSermonInit(false);
  var href = window.location.href;
  if (href.indexOf('#') > 0) {
    var sermon = href.substring(href.indexOf('#') + 1);
    showSermon(sermon, true);
  } else {
...
function animatedSermonInit(scrollBody) {
  ...
  if (scrollBody) {
    ...
    window.setInterval(function() {
      $('#sermonframe')
          .animate({scrollTop: $('#sermonframe')[0].scrollHeight}, 'fast');
    }, 500);
  }
}
...
function nextColorForId(itemId) {
  ...
  if (currentColor == '') currentColor = $(itemId).css('border-left-color');
...
function typetext(append_element, textarray, donecallback) {
  ...
  var f = function(index) {
    if (localCurrLine != currLine) return;
    if (index < shit.length)
      type(append_element, shit[index], function() {
        f(index + 1);
      });
    else {
      if (donecallback) {
...
function type(append_element, text, donefunc) {
  ...
  if (/XXX/.exec(text)) {
    ...
    $(element).typed(
        {strings: [text], typeSpeed: 0, showCursor: false, callback: donefunc});
  }
}
...
sermons.intro =
    'Inspired by his never-ending quest for progress, in |1=2084 |man perfects the |1=Robotrons: \
...
sermons.what =
    'Members of the human family, hear me! ^500The |1=Robotrons| are coming! ^400Yes,^300 |1=the Robotrons are coming! \
...
sermons.how =
    'Only the Mutant Savior will have the power to stop a |1=Robotron.\
...
sermons.ninth =
    'Members of the human family who decide to take up the two 8-ways must remain vigilant against those who would try to lead them to the |2=ninth way\
...
function showSermon(which, autostart) {
  ...
  var newIframe = '<iframe src="' + videos[which] + autoText +
      ' width="500" height="369" frameborder="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe>';
