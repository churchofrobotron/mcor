...
def find_devices():
   ...
   for dev_name in potential_devices:
      try:
         serial_devices.append(serial.Serial(dev_name, baudrate=3500000))
      except:
...
def send_wave_vr():
   send_command("WaveNum2\n")

def send_heartbeat(num):
   send_command("BEAT1:" + "{0:x}\n".format(int(num)))

def send_player_killed():
   send_command("3on\n")

def send_player_killed_over():
   send_command("3off\n")
...
def parse_scoreboard(msg):
    ...
    print requests.post(scoreboard_url, data = payload, files = files, timeout=6.05)
...
def start_capture():
   ...
   if (capture_handle == None):
      capture_handle = cv.CaptureFromCAM(2)
   capturing = True
   capture_if_needed()
...
def power_on_test():
   ...
   send_player_killed()
   time.sleep(1)
   send_player_killed_over()
   send_wave_main()
   send_wave_vr()
   stop_capture()

def open_incoming_port(port):
...
#      dump_udp = (int(argv[2]) == 1)
   ...
   s = open_incoming_port(2086) # Should be 2086
   s2 = open_incoming_port(2085)

   light2 = None
   strobe = None
   scan_time = 2
   scan = time.time() - scan_time

   print "Waiting for data..."
   while True:
      try:
         # Commented out for quick VR hack, we'll need to re-enable this for effects.
         if (time.time() - scan >= scan_time):
            find_devices()
            scan = time.time()

         capture_if_needed()
         if light2 is not None:
            if (time.time() - light2) > 2:
               send_command("2off\n")
               light2 = None

         if strobe is not None:
            if (time.time() - strobe) > 1.5:
               send_player_killed_over()
               strobe = None
